#!/usr/bin/env python3
"""
ESPN-based prior week analysis system.
This analyzes Week N-1 results using free ESPN API to improve Week N picks.
"""
import os
import json
import pandas as pd
...
class ESPNPriorWeekAnalyzer:
    """Analyze prior week results using ESPN API to improve current week picks"""
    ...
    def analyze_prior_week(self, year: int, week: int) -> Dict:
        ...
        # Load our predictions for the prior week
        ...
        analysis = self._calculate_accuracy_metrics(matched_games, year, prior_week)
    ...
    def _calculate_accuracy_metrics(self, matched_games: List[Dict], year: int, week: int) -> Dict:
    ...
    def _calculate_confidence_adjustments(self, all_analyses: List[Dict], current_week: int) -> Dict:
        ...
        for conf_points, data in confidence_data.items():
            if data["total_weight"] > 0:
                ...
                conf_points_int = int(conf_points)  # Convert to int
                expected_accuracy = 0.5 + (conf_points_int - 1) * 0.02  # Rough expected accuracy
...
def main():
    """Test the ESPN prior week analyzer"""
    analyzer = ESPNPriorWeekAnalyzer()
    ...
    if analysis:
        ...
        if adjustments:
            ...
            for conf_points, factor in sorted(adjustments.items()):
                print(f"  {int(conf_points):2d} points: {factor:.2f}x adjustment")
    else:
