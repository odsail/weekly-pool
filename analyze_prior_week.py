#!/usr/bin/env python3
"""
Analyze prior week results to improve current week picks.
This compares our predictions against actual results and calculates accuracy metrics.
"""
import json
import os
import sys
...
class PriorWeekAnalyzer:
    """Analyzes prior week results to improve future predictions"""
    
    def __init__(self):
        self.espn_api = ESPNAPI()
        self.accuracy_history = []
    
    def analyze_week(self, year: int, week: int) -> Dict:
        """
        Analyze a specific week's results against our predictions.
        ...
        """
        print(f"🔍 Analyzing Week {week} results...")
        
        # Load our predictions for this week
        predictions = self._load_week_predictions(year, week)
        if not predictions:
            print(f"❌ No predictions found for Week {week}")
            return {}
        
        # Get actual results from ESPN
        actual_results = self.espn_api.get_week_results(year, week)
        ...
        analysis = self._calculate_accuracy_metrics(matched_games)
        
        # Save analysis for future use
        self._save_analysis(year, week, analysis)
        
        return analysis
    
    def get_accuracy_adjustments(self, current_week: int) -> Dict:
        """
    ...
    def _calculate_accuracy_metrics(self, matched_games: List[Dict]) -> Dict:
        """Calculate accuracy metrics for the matched games"""
        if not matched_games:
            return {}
...
def main():
    """Test the prior week analyzer"""
    analyzer = PriorWeekAnalyzer()
    
    # Analyze Week 1 (since we're in Week 2)
    analysis = analyzer.analyze_week(2025, 1)
    ...
    if analysis:
        ...
        adjustments = analyzer.get_accuracy_adjustments(2)
        if adjustments:
